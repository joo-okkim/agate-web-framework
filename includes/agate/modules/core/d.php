...
//get the top 10 posts from db:
			if(isset($_GET['id'])) {
				...
				echo($aAttributes['headerList']);
				echo(self::dbAtos(
					"SELECT "
						."DATE_FORMAT(`date`, '%Y') AS `year`, "
						."`title` AS `title`, "
						."(SELECT `value` FROM `postmeta` WHERE `post`= `posts`.`id` AND `key` = 'period') AS `date`, "
						."(SELECT `value` FROM `postmeta` WHERE `post`= `posts`.`id` AND `key` = 'thumbnail') AS `thumbnail`, "
						."`excerpt` AS `excerpt`, "
						."`permalink` AS `permalink`, "
						."CASE `key` "
							."WHEN 'CD6EA6A0-42A9-419F-8DFA-3B622AD26994' THEN 'centru' "
							."ELSE 'x' END AS `category`"
					."FROM `posts` "
					."WHERE `key` = 'CD6EA6A0-42A9-419F-8DFA-3B622AD26994' "
					."ORDER BY `date` DESC", $aAttributes['patternList']));
				echo($aAttributes['footerList']);
			}
			break;
		case 'item':
...
//do nothing;
		...
		if(isset($aAttributes['key'])) {
			self::$arSections[] = $aAttributes['key'];
		}
	}
